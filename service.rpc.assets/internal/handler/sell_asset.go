...
func ValidateSellAssetRequest(req *assetsproto.SellAssetRequest) error {
...
func (h Handler) SellAsset(ctx context.Context, req *assetsproto.SellAssetRequest) (*assetsproto.Asset, error) {
	if err := ValidateSellAssetRequest(req); err != nil {
	...
	if err := h.conn.RunInTransaction(ctx, func(tx *pg.Tx) error {
		...
		h.ordersClient.CreateOrder(ctx, &ordersproto.CreateOrderRequest{
			...
			Order: &ordersproto.Order{
				...
				Instruction: ordersproto.Instruction_INSTRUCTION_SELL,
				Amount:      req.Units,
